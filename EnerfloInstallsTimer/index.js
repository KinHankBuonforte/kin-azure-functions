        ...
        for (let key of Object.keys(obj)) {
            ...
            if (
                ...
                for (let valueKey of Object.keys(value)) {
                    ...
                    if (
                        (typeof valueValue === 'object' ||
                            Array.isArray(valueValue)) &&
                        valueValue !== null
                    ) {
                        flattened[key + '_' + valueKey] =
    ...
    const initTable = async (connection, tableName, records) => {
        ...
        for (let column of columns) {
            if (column.values.every((v) => typeof v == 'boolean')) {
                ...
                if (
                    column.values.some((x) => x.toString().indexOf('.') != -1)
                ) {
                    column.type = 'FLOAT';
                } else {
                    column.type = 'INT';
        ...
        const existingColumns = await executeSql(
            connection,
            `SELECT COLUMN_NAME FROM ENERFLO.INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='${tableName}'`
        );
        if (existingColumns.length > 0) {
            ...
            for (let column of columns) {
                if (
                    !existingColumns.some(
                        (x) => x['COLUMN_NAME'] == column.name.toUpperCase()
                    )
                ) {
                    await executeSql(
                        connection,
                        `ALTER TABLE ${tableName} ADD ${column.name} ${column.type}`
                    );
                }
            }
        } else {
            ...
            await executeSql(
                ...
                `CREATE TABLE IF NOT EXISTS ${tableName} (${columns
                    .map((x) => `${x.name.toUpperCase()} ${x.type}`)
                    .join(', ')})`
            );
        }
    ...
    for (let i = 0; i < flattenRecords.length; i += 100) {
        ...
        const values = flattenRecords
            ...
            .map(
                (record) =>
                    `(${columns
                        .map((x) => sqlString(record[x.toLowerCase()]))
                        .join(', ')})`
            )
            .join(', ');
        await executeSql(
